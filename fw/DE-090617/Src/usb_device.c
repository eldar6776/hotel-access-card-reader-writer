...
USB_HANDLE USBTransferOnePacket(BYTE ep, BYTE dir, BYTE* data, BYTE len)
{
    ...
    if (dir != 0)
    {
        handle = pBDTEntryIn[ep]; //point to the IN BDT of the specified endpoint
    }
    else
    {
        handle = pBDTEntryOut[ep]; //else point to the OUT BDT of the specified endpoint
    }
    ...
    {
        USBAdvancePingPongBuffer(&pBDTEntryIn[ep]); //toggle over the to the next buffer for an IN endpoint 
    }
    else
    {
        USBAdvancePingPongBuffer(&pBDTEntryOut[ep]); //toggle over the to the next buffer for an OUT endpoint 
    }
